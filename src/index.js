...
const extractedTextWithLocation = [];
...
for (const link of extractedTextWithLocation) {
	...
	const filtered = files.filter((file) =>
		file.endsWith(link.text.split("-")[0] + ".pdf"),
	);
	...
	const res = indexPDFDoc.context.register(
		indexPDFDoc.context.obj({
			...
			A: {
				...
				S: "URI",
				URI: PDFString.of(
					`${docsBuild ? "https://drive.google.com/viewerng/viewer?url=https://colecrouter.github.io/evo-manual-fix/google/" : "./"}${fileName}#page=${Number(link.text.split("-")[1]) - pageOffset}`,
				),
			},
		}),
	);
...
await writeFile(
	"./output/" + (docsBuild ? "google/" : "") + "INDEX.pdf",
	bytes,
);
